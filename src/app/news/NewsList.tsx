...
export default function NewsList({ items, categories }: { items: NewsItem[]; categories: NewsCategoryMeta[] }) {
  ...
  return (
    <>
      ...
      {urgent.length > 0 && (
        <div className="mb-6">
          ...
          <div className="space-y-3">
            {urgent.map(item => (
              <NewsCard key={item.id} item={item} categories={categories} expanded={expandedId === item.id} onToggle={() => setExpandedId(expandedId === item.id ? null : item.id)} highlight />
            ))}
          </div>
        </div>
      )}
      ...
      <div className="space-y-3">
        ...
        {rest.map(item => (
          <NewsCard key={item.id} item={item} categories={categories} expanded={expandedId === item.id} onToggle={() => setExpandedId(expandedId === item.id ? null : item.id)} />
        ))}
      </div>
    </>
  );
...
function NewsCard({ item, categories, expanded, onToggle, highlight }: { item: NewsItem; categories: NewsCategoryMeta[]; expanded: boolean; onToggle: () => void; highlight?: boolean }) {
