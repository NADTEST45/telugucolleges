  ...
  return (
    <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
      ...
      {colleges.length < 2 ? (
        ...
        <>
        <div className="sm:hidden text-xs sm:text-[11px] text-gray-500 text-center mb-1.5">Swipe to see all columns →</div>
        <div className="overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0 scrollbar-hide">
          <table className="w-full bg-white rounded-xl overflow-hidden shadow-sm text-sm min-w-[500px]">
            <thead>
              <tr>
                <th className="px-3 sm:px-4 py-2.5 sm:py-3 bg-brand text-white text-left text-[11px] sm:text-xs min-w-[90px] sm:min-w-[140px] sticky left-0 z-10">Feature</th>
                {colleges.map(c => (
                  <th key={c.id} className="px-2.5 sm:px-3 py-2.5 sm:py-3 bg-brand text-white text-center min-w-[100px] sm:min-w-[150px]">
                    ...
                    <div className="text-[11px] opacity-80 font-normal mt-0.5">{c.name.length > 20 ? c.name.slice(0, 20) + "..." : c.name}</div>
                  </th>
                ))}
              </tr>
            </thead>
            <tbody>
              ...
              {/* Fees */}
              <tr><td colSpan={colleges.length + 1} className="px-3 sm:px-4 py-2 sm:py-2.5 bg-blue-50 font-bold text-brand text-[11px] sm:text-xs">Fee Structure (Annual)</td></tr>
              {(() => {
              ...
              {/* Placements */}
              <tr><td colSpan={colleges.length + 1} className="px-3 sm:px-4 py-2 sm:py-2.5 bg-blue-50 font-bold text-brand text-[11px] sm:text-xs">Placements</td></tr>
              {[
