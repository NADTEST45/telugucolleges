...
export default function Home() {
  ...
  return (
    <main>
      {/* Hero */}
      <section className="bg-gradient-to-br from-[#0f2b3d] via-[#1a5276] to-[#2e86c1] text-white px-6 pt-16 pb-20 text-center relative overflow-hidden">
        <div className="absolute inset-0 opacity-[0.04]" style={{backgroundImage: "radial-gradient(circle at 1px 1px, white 1px, transparent 0)", backgroundSize: "32px 32px"}} />
        <div className="relative max-w-3xl mx-auto">
          <p className="text-sm uppercase tracking-widest text-blue-200 mb-3 font-medium">Andhra Pradesh & Telangana</p>
          <h1 className="text-3xl sm:text-5xl font-extrabold mb-5 leading-tight">
            Find the Right College<br />for Your Future
          </h1>
          <p className="text-lg opacity-80 mb-10 font-light max-w-xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/colleges" className="inline-block bg-white text-[#1a5276] font-bold px-8 py-3.5 rounded-xl text-lg shadow-xl hover:shadow-2xl transition-all hover:-translate-y-0.5">
              Explore Colleges
            </Link>
            <Link href="/eapcet" className="inline-block bg-white/15 text-white font-bold px-8 py-3.5 rounded-xl text-lg border border-white/30 hover:bg-white/25 transition-all hover:-translate-y-0.5">
              EAPCET Predictor
            </Link>
          </div>
        </div>
      ...
      <nav className="bg-white border-b border-gray-100 shadow-sm sticky top-0 z-20">
        <div className="max-w-7xl mx-auto flex items-center gap-1 overflow-x-auto scrollbar-hide px-4 sm:px-6">
          {[
            ...
            <Link key={item.label} href={item.href}
              className="px-3 sm:px-4 py-3 text-xs sm:text-sm font-medium text-gray-600 hover:text-[#1a5276] hover:bg-blue-50 whitespace-nowrap transition-colors border-b-2 border-transparent hover:border-[#2e86c1]">
              {item.label}
            </Link>
          ))}
        </div>
      ...
      {/* Browse by Category */}
      <section className="max-w-7xl mx-auto px-6 -mt-8 relative z-10 pt-12">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        {/* Top CSE Colleges */}
        <h2 className="text-2xl font-bold mb-1">Top CSE Colleges by EAPCET Cutoff</h2>
        <p className="text-sm text-gray-500 mb-6">Final-phase OC closing ranks from official TSCHE/APSCHE data. Lower rank = harder to get in.</p>
        <div className="grid md:grid-cols-2 gap-6 mb-14">
          {[
            ...
            <div key={title} className="bg-white rounded-xl overflow-hidden shadow-sm">
              <div className={`${color} text-white px-5 py-3.5 font-bold`}>{title}</div>
              {list.map((c, i) => (
                <Link key={c.id} href={`/colleges/${c.slug}`} className="flex items-center justify-between px-5 py-3 border-b border-gray-100 hover:bg-gray-50 transition-colors">
                  <div>
                    <span className="text-gray-400 font-semibold mr-2 text-sm">#{i + 1}</span>
                    <span className="font-semibold text-sm">{c.name.length > 40 ? c.name.slice(0, 40) + "..." : c.name}</span>
                  </div>
                  <div className="text-right">
                    <div className="font-bold text-[#1a5276] text-sm">Rank {c.cutoff.cse.toLocaleString()}</div>
                    <div className="text-xs text-gray-400">{fmtFee(c.fee)}/yr</div>
                  </div>
                </Link>
              ))}
            </div>
        ...
        {/* Affordable Colleges */}
        <h2 className="text-2xl font-bold mb-1">Most Affordable B.Tech Programs</h2>
        <p className="text-sm text-gray-500 mb-6">Lowest tuition fees across AP & Telangana.</p>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-14">
          {cheapest.map(c => (
            <Link key={c.id} href={`/colleges/${c.slug}`} className="bg-white rounded-xl p-5 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all">
              <div className={`text-xs font-semibold mb-1 ${c.type === "Government" ? "text-green-600" : "text-[#2e86c1]"}`}>
                {c.type} · {c.state}
              </div>
              <div className="font-bold mb-2 leading-snug">{c.name}</div>
              <div className="flex items-center justify-between">
                <div className="text-xl font-extrabold text-[#1a5276]">{fmtFee(c.fee)}<span className="text-xs font-normal text-gray-400">/yr</span></div>
                <span className="bg-blue-50 text-[#2e86c1] px-2.5 py-1 rounded-md text-xs font-semibold">{c.district}</span>
              </div>
            </Link>
          ))}
        </div>
        ...
        <div className="grid sm:grid-cols-3 gap-5 mb-14">
          {[
            ...
            <Link key={title as string} href={href as string} className={`block bg-white rounded-xl p-6 shadow-sm border-l-4 ${border} hover:shadow-md hover:-translate-y-0.5 transition-all`}>
              <div className="font-bold text-lg mb-1">{title as string}</div>
              <div className="text-sm text-gray-500">{desc as string}</div>
            </Link>
          ))}
        </div>
        ...
        <section className="bg-white rounded-2xl p-8 shadow-sm">
          <h2 className="text-2xl font-bold text-center mb-6">Built for Telugu States</h2>
