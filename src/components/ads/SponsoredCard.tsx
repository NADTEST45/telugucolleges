...
export default function SponsoredCard({ ad, variant = "horizontal", className = "" }: SponsoredCardProps) {
  if (variant === "vertical") {
    return (
      <div className={`bg-gradient-to-br from-indigo-50 to-white border border-indigo-100 rounded-xl p-5 shadow-sm relative ${className}`}>
        <span className="absolute top-3 right-3 bg-indigo-100 text-indigo-600 text-[11px] font-bold uppercase tracking-wider px-2 py-0.5 rounded">
          {ad.label || "Sponsored"}
        </span>
        ...
        {ad.badgeText && (
          <span className="inline-block px-2 py-0.5 rounded text-[11px] font-bold bg-amber-50 text-amber-700 mb-3">
            {ad.badgeText}
          </span>
        )}
  ...
  return (
    <div className={`bg-gradient-to-r from-indigo-50 to-white border border-indigo-100 rounded-xl px-4 sm:px-6 py-4 shadow-sm relative ${className}`}>
      <span className="absolute top-2.5 right-3 bg-indigo-100 text-indigo-600 text-[11px] font-bold uppercase tracking-wider px-2 py-0.5 rounded">
        {ad.label || "Sponsored"}
      </span>
      ...
      <div className="flex flex-col sm:flex-row gap-4 sm:items-center">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div className="min-w-0">
            <div className="text-[11px] text-indigo-600 font-semibold uppercase tracking-wide">{ad.tagline}</div>
            ...
            <div className="flex gap-3 mt-2 flex-wrap">
              {ad.highlights.slice(0, 4).map((h, i) => (
                <span key={i} className="text-[11px] text-gray-600 font-medium flex items-center gap-1">
        ...
        <div className="flex items-center gap-3 shrink-0">
          {ad.badgeText && (
            <span className="hidden sm:inline-block px-2 py-0.5 rounded text-[11px] font-bold bg-amber-50 text-amber-700">
              {ad.badgeText}
            </span>
          )}
          <a
